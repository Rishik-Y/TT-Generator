...
def seed_users():
    ...
    cur = conn.cursor()
    print(f"  ✓ Connected to {os.getenv('DB_NAME', 'timetable_generator_db')}")
    ...
    if admin_uid:
        ...
        print(f"  ✓ Admin: {ADMIN_EMAIL} (UID: {admin_uid[:12]}...)")
    ...
    print(f"\n  ✓ {created} faculty accounts created/updated")
    if skipped:
        print(f"  ⚠ {skipped} accounts skipped (errors)")
    print(f"  ✓ 1 admin account created ({ADMIN_EMAIL})")
